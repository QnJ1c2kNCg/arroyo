//! Preserve the old FileIO's local paths and S3 addressing defaults while using
//! the new pluggable storage API. Do not rewrite paths in existing manifests.
...
mod tests {
    ...
    #[test]
    fn path_style_is_only_a_fallback() {
        for (supplied, expected) in [
            ...
            if let Some(value) = supplied {
                config = config.with_prop("s3.path-style-access", value);
            }
            let storage = ArroyoStorageFactory.build(&config).unwrap();
            let encoded = serde_json::to_value(&storage).unwrap();
            assert_eq!(encoded["remote"]["props"]["s3.path-style-access"], expected);
            let decoded: Arc<dyn Storage> = serde_json::from_value(encoded.clone()).unwrap();
