...
use datafusion::functions_aggregate::{approx_percentile_cont, approx_percentile_cont_with_weight};
...
impl OperatorConstructor for IncrementalAggregatingConstructor {
    ...
    fn with_config(
        ...
        let aggregates: Vec<_> = aggregate_exec
            ...
            .map(|(expr, name)| {
                let agg = decode_aggregate(
                    &input_schema.schema,
                    name,
                    expr,
                    &task_context,
                )?;
                // Ordered-set percentiles sort their own inputs; the ORDER BY
                // direction remains in the expression passed to their factory.
                // Other order-sensitive aggregates cannot use this unordered cache.
                let udf = agg.fun().inner();
                let sorts_inputs = udf.downcast_ref::<approx_percentile_cont::ApproxPercentileCont>().is_some()
                    || udf.downcast_ref::<approx_percentile_cont_with_weight::ApproxPercentileContWithWeight>().is_some();
                if !agg.order_bys().is_empty() && !sorts_inputs {
                    bail!(
                        "ORDER BY in updating aggregates is not supported: {}",
                        agg.name()
                    );
                }
...
mod tests {
    ...
    use datafusion::functions_aggregate::{count, min_max, sum};
